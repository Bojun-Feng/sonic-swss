...
bool RouteOrch::evictRoutesOnIntf(const string& alias, int refCount, size_t &evicted)
{
    ...
    for (const auto& route : routes)
    {
         ...
         * can be replayed. The ZMQ route channel does not persist the main hash
         * (dbPersistence=false), and the hash is a field union: fpmsyncd omits
         * fields at their default value, and a pop only overwrites the fields
         * it carries, so fields from an earlier shape of the prefix survive and
         * would program something else entirely. A task already parked for the
         * key carries newer intent and must not be duplicated, the cache holds
         * at most a DEL and a SET per key. */
        ...
        if (!mainHashTrusted || ifname != alias || !nexthop_group.empty() || blackhole == "true" ||
            retryCache == nullptr || retryCache->getRetryMap().count(key) != 0)
        {
            SWSS_LOG_NOTICE("Route %s on interface %s cannot be replayed after eviction, skip eviction",
