...
namespace intfsorch_test
{
    ...
    TEST_F(IntfsOrchTest, IntfsOrchEvictMultipleRoutesInOnePass)
    {
        ASSERT_NO_FATAL_FAILURE(createIntf("Ethernet0"));
        ASSERT_NO_FATAL_FAILURE(programIntfRoute("10.20.1.0/24", "Ethernet0", m_app_db.get()));
        ASSERT_NO_FATAL_FAILURE(programIntfRoute("10.20.2.0/24", "Ethernet0", m_app_db.get()));
        ASSERT_EQ(gIntfsOrch->getSyncdIntfses().at("Ethernet0").ref_count, 2);

        // Both routes account for the whole reference count, so one pass
        // evicts and parks both
        auto base_remove_route = remove_route_entries_count;
        std::deque<KeyOpFieldsValuesTuple> entries;
        entries.push_back({"Ethernet0", "DEL", { {} }});
        auto intfConsumer = dynamic_cast<Consumer *>(gIntfsOrch->getExecutor(APP_INTF_TABLE_NAME));
        ...
        ASSERT_FALSE(gRouteOrch->isRouteExists(gVirtualRouterId, IpPrefix("10.20.1.0/24")));
        ASSERT_FALSE(gRouteOrch->isRouteExists(gVirtualRouterId, IpPrefix("10.20.2.0/24")));
        ASSERT_EQ(base_remove_route + 2, remove_route_entries_count);
        ASSERT_EQ(gIntfsOrch->getSyncdIntfses().at("Ethernet0").ref_count, 0);
        auto rc = gRouteOrch->getRetryCache(APP_ROUTE_TABLE_NAME);
        ASSERT_EQ(rc->getRetryMap().size(), 2u);
        ...
        ASSERT_NO_FATAL_FAILURE(createIntf("Ethernet0"));
        static_cast<Orch *>(gRouteOrch)->doTask();
        ASSERT_TRUE(gRouteOrch->isRouteExists(gVirtualRouterId, IpPrefix("10.20.1.0/24")));
        ASSERT_TRUE(gRouteOrch->isRouteExists(gVirtualRouterId, IpPrefix("10.20.2.0/24")));
        ASSERT_EQ(gIntfsOrch->getSyncdIntfses().at("Ethernet0").ref_count, 2);
        ASSERT_TRUE(rc->getRetryMap().empty());
    }
    ...
    TEST_F(IntfsOrchTest, IntfsOrchNoEvictionWhileAddressesRemain)
    {
        ...
        // The address DEL sorts after the interface DEL, so this drain removes
        // the address and the retained interface DEL is still refused for it
        entries.clear();
        entries.push_back({"Ethernet0:10.88.0.1/24", "DEL", {}});
        intfConsumer->addToSync(entries);
        static_cast<Orch *>(gIntfsOrch)->doTask();
        ASSERT_TRUE(gIntfsOrch->getSyncdIntfses().at("Ethernet0").ip_addresses.empty());
        ASSERT_EQ(base_remove_route, remove_route_entries_count);

        // Only now does the removal reach the reference check, and the route
        // wedge is repaired
        static_cast<Orch *>(gIntfsOrch)->doTask();
    ...
    TEST_F(IntfsOrchTest, IntfsOrchEvictSkipsStaleReplayIntent)
    {
        ASSERT_NO_FATAL_FAILURE(createIntf("Ethernet0"));
        ...
        // earlier shape of the prefix would replay as a different route, so
        // the route must not be evicted on the strength of it
        Table appRouteTable(m_app_db.get(), APP_ROUTE_TABLE_NAME);
        ...
        // Fail-safe: the wedge persists exactly as it does today
        ASSERT_TRUE(gRouteOrch->isRouteExists(gVirtualRouterId, IpPrefix("10.10.12.0/24")));
        ASSERT_EQ(base_remove_route, remove_route_entries_count);
        ASSERT_TRUE(gIntfsOrch->isIntfChangeInProgress("Ethernet0"));
        auto rc = gRouteOrch->getRetryCache(APP_ROUTE_TABLE_NAME);
        ASSERT_TRUE(rc->getRetryMap().empty());

        // A normal route DEL still converges the removal
        appRouteTable.del("10.10.12.0/24");
        auto routeConsumer = dynamic_cast<Consumer *>(gRouteOrch->getExecutor(APP_ROUTE_TABLE_NAME));
        std::deque<KeyOpFieldsValuesTuple> routeEntries;
        routeEntries.push_back({"10.10.12.0/24", "DEL", {}});
        routeConsumer->addToSync(routeEntries);
        static_cast<Orch *>(gRouteOrch)->doTask();
        static_cast<Orch *>(gIntfsOrch)->doTask();
        ASSERT_FALSE(gIntfsOrch->isIntfChangeInProgress("Ethernet0"));
    ...
    {
        // The VRF rehome sequence end to end: an interface is unbound from a
